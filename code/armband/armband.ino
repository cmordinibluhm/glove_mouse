...
bool print_packet = true; // true to print packets to serial monitor for debugging
bool print_mouse_actions = true; // true to print click messages to serial monitor
 ...
 */
#define inter_finger_pad_pin 27           // middle click       (0x20 on Uno PINB)(0x20 Mega PINA)
#define pointer_pad_pin 26                // move enable        (0x10 on Uno PINB)(0x10 Mega PINA)
#define middle_pad_pin 25                 // scroll enable      (0x08 on Uno PINB)(0x08 Mega PINA)
#define thumb_pad_pin 24                  // rapid fire enable  (0x04 on Uno PINB)(0x04 Mega PINA)
  ...
  // Every other 8 ms, transmit the cursor position or scroll message
    ...
    if (scroll_en == 1) {
      ...
      if (scrolltime == 90) {
        scrolltime = 0;
        if (move_y > 128) {
          tx_packet(0x00, 0x00, 0x01, 0x00);
          Serial.println(move_y);
        } else if (move_y < 128) {
          Serial.println(move_y);
          tx_packet(0x00, 0x00, 0xFE, 0x00);
        }
      }
    }
  ...
  switch(PushState) {
    ...
    case MaybePush: 
      if (butnumPressed == currentButton) { // If same button still pressed, it must be a valid press
        ...
        else if (currentButton == PIN11) { // If pin 11 pressed (POINTER PAD) __________ move_en
        ...
        } else if (currentButton == PIN10) { // else if (THUMB PAD) _____ rapid_fire_en
          rapid_fire_en ^= 1;
          if (rapid_fire_en == 1) {
